...
void Groups(list <data>& studs, list <data>& Geri, list<data>& Blogi) {
  ...
  copy(it, studs.end(), back_inserter(Geri));
  
  copy(studs.begin(), it, back_inserter(Blogi));
  
}
void Sortlist(list <data>& studs, int sorting) {
  if (sorting == 1)
    studs.sort([](data& f, data& s) {

...
void output(data& temp) {
    ...
    if (vm) {
        ...
        if (temp.mediana.size() % 2 != 0){
            temp.galutinis = 0.4 * ((double)temp.mediana[(temp.mediana.size()) / 2]) + 0.6 * temp.egz;
            cout << setw(20) << std::left << temp.vardas << setw(20) << temp.pavarde << setw(20) << std::setprecision(3) << temp.galutinis << endl;
        }
        else
            {
            temp.galutinis = 0.4 * ((double)(temp.mediana[(temp.mediana.size()) / 2] + 
            temp.mediana[(temp.mediana.size() - 1) / 2]) / 2.0 ) + 0.6 * temp.egz;
            cout << setw(20) << std::left << temp.vardas << setw(20) << temp.pavarde << setw(20) << std::setprecision(3) << temp.galutinis << endl;
        }
    }
}
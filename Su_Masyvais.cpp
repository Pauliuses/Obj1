...
void Median(data& temp) {
    ...
    std::sort(temp.mediana, temp.mediana + (temp.pazymiuSkaicius));
}
void Input(data& temp) {
    ...
    temp.mediana = new int[temp.pazymiuSkaicius];
    if (temp.gen == 0) {
        for (int i = 0; i < temp.pazymiuSkaicius; i++) {
...
void output(data& temp) {
    ...
    if (vm) {
        ...
        if (temp.pazymiuSkaicius % 2 != 0){
            temp.galutinis = 0.4 * ((double)temp.mediana[(temp.pazymiuSkaicius) / 2]) + 0.6 * temp.egz;
            cout << setw(20) << std::left << temp.vardas << setw(20) << temp.pavarde << setw(20) << std::setprecision(3) << temp.galutinis << endl;
        }
        else
        {
            temp.galutinis = 0.4 * ((double)(temp.mediana[(temp.pazymiuSkaicius) / 2] + 
            temp.mediana[(temp.pazymiuSkaicius - 1) / 2]) / 2.0 ) + 0.6 * temp.egz;
            cout << setw(20) << std::left << temp.vardas << setw(20) << temp.pavarde << setw(20) << std::setprecision(3) << temp.galutinis << endl;
        }

...
void ReadFromFile(deque<data>& list, string filename)
{
...
double AverageGrade(deque<int> paz)
{
    double sum = 0;
...
double MedianGrade(deque<int> paz)
{
    double vidurkis;
...
void FinalResult(deque<data>& list, bool AvgMed)
{
    for (auto& a : list)
        if (AvgMed == 1)
...
void Output(deque<data>& list, string filename, int vm)
{
...
void Groups(deque<data>& list, deque<data>& Geri, deque<data>& Blogi)
{
    ...
    copy(index, list.end(), back_inserter(Geri));

    copy(list.begin(), index, back_inserter(Blogi));
}
